...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method == 'POST':
            ...
            if sno == "0":
                post = Posts(title=req_title, tagline=tagline, slug=slug, content=content, img_file=img_file,
                             date=date)
                db.session.add(post)
                db.session.commit()
...
def contact():
    if request.method == 'POST':
        ...
        """sno,name,email,phone_no,message,date"""
        entry = Contacts(name=name, email=email, phone_no=phone, message=msg, date=datetime.now())
